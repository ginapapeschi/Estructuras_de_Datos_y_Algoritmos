...
class Dirafo:
    ...
    def insertar(self, verticeInicial, verticeFinal):
        ...
        if self.__matriz[indiceInicial][indiceFinal] == 0:
            ...
            # Cambia con grafo en que es unidireccional, no bidireccional. Por eso no se establece la recíproca.
        else:
    ...
    def topologia(self):
        L = []
        if not self.esAciclico():
            print("No se puede obtener el orden topológico ya que el grafo contiene ciclos")
        else:
            # Inicializo estructuras globales
            descubrimiento = [0] * self.__tamaño
            fin = [0] * self.__tamaño
            tiempo = [0]
    
            # Llamo a BEPVisita desde todos los vértices no visitados
            for i in range(self.__tamaño):
                if descubrimiento[i] == 0:
                    self.BEPVisita(i, descubrimiento, fin, tiempo)
    
            # Tu bloque original de ordenamiento
            for i in range(self.__tamaño):
                verticeActual = self.__vertices[i]
                tiempoActual = fin[i]
                insertado = False
                posicion = 0
    
                while posicion < len(L):
                    verticeLista = L[posicion]
                    tiempoLista = fin[self.__vertices.index(verticeLista)]
                    if tiempoActual > tiempoLista and not insertado:
                        L.insert(posicion, verticeActual)
                        insertado = True
                    posicion += 1
    
                if not insertado:
                    L.append(verticeActual)
        return L
